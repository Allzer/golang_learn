...
--------------------
57) Горутины - легковесные процессы внутри golang. Могут выполняться параллельно и не занимают много ресурсов

        func func1() {
            fmt.Println("Началась горутина 1")
            time.Sleep(time.Second)
            fmt.Println("Горутина 1 выполнена")
        }

        func main() {
            fmt.Println("Start main gorutin")
            go func1()
            fmt.Println("Горутина продолжает работать")

            time.Sleep(2 * time.Second)
            fmt.Println("Всё выполнено")
        }

58) Канал - интрумент для обмена данными между горутинами.
    ...
    Расширеный пример:
        ...
        func main() {
            ...
            //Горутина, которая отправляет дзначения в канал
            go func() {
                for i := 1; i <= 5; i++ {
...
59) WaitGroup - механизм синхронизации выполнения горутин. Он гарантирует, что все горутины завершат работу, прежде чем программа завершится
    ...
    Расширенынй пример:
        ...
        func worker (id int, wg *sync.WaitGroup) {
            ...
            fmt.Printf("Горутина %d завершила совю работу \n", id)
        }
        ...
        func main() {
            ...
            for i:=1; i<=3; i++ {
                wg.Add(1) //увеличение счётчика горутн на 1
                go worker(i, &wg) //запуск горутины
            }

            wg.Wait() //ожидание завершения всех горутин

            fmt.Println("Все горутны заверщили работу")
        }

60) Оператор Select. Позволяет выбирать из нескольких операций над каналами; позволяет реагировать на доступные данные из разных горутин одновременно
